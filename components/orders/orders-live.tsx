...
import { isMuted, playBeep, unlockAudio } from "@/lib/beep"
...
export function OrdersLiveProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (!isMuted()) {
      // One beep however many arrived at once. Four beeps is not four times
      // the information, it is noise.
      try {
        playBeep(true)
      } catch {
        /* no audio context yet — the badge and the title still carry it */
      }
    }
